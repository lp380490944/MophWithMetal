...
//    float4 修饰符，是一个4维向量，[[position]]是内建修饰符，不可更改
    ...
    int effectType;
//    颜色，相当于OpenGL ES中的gl_FragColor //此处的颜色字段需要保留，因为从顶点函数生成的颜色需要传递到片元函数，
    half4 color;
...
vertexShader(uint vertexID [[vertex_id]],
             device const CJLVertex *vertices [[buffer(CJLVertexInputIndexVertices)]],
             constant vector_uint2 *viewportSizePointer [[buffer(CJLVertexInputIndexViewportSize)]],constant int &effectType[[buffer(CJLFragmentInputIndexEffectType)]] )
{
        /*
        处理顶点数据:
...
//    3、获取当前顶点坐标的xy，因为是2D图形
    // 索引到我们的数组位置以获得当前顶点
    // 我们的位置是在像素维度中指定的.
    CJLVertex currentVertex = vertices[vertexID];
    float2 pixelSpacePosition = currentVertex.position.xy;
...
fragment half4 fragmentShader(RasterizerData in [[stage_in]],device float &colorShit[[buffer(CJLFragmentInputIndexColorShift)]])
{
...
//    log(in.clipSpacePosition);
    ...
    half4 color = half4(rgbColor.r,rgbColor.g,rgbColor.b,1.0);
    return color;
}
